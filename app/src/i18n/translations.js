module.exports = {
  en: {
    translation: {
      'How to Vote': 'How to Vote',
      'Map': 'Map',
      'About': 'About',
      'Vot Diaspora': 'Vote From Abroad',
      'HeroBody': '<p>On November 10, 2019, presidential elections are taking place in Romania. Romanians everywhere have the right to choose their next president. If you live abroad, you can vote by mail or at the local polling station. Find out by using <i>Vot Diaspora</i> what procedure you must follow for registering to vote by mail or what documents you need to vot at the station.</p><p>If you decided to vote at your local station, it doesn\'t matter you are a resident or just visiting, you can use the map below to see which polling sations are closest to you. The presidential election will be taking place during three days for each voting round.</p>',
      'HowToVoteBody': '???',
      'Map of Polling Stations Abroad': 'Map of Polling Stations Abroad',
      'MapMessage': 'Use the map below to find what is the closest polling station for you.',
      'Requestion a new station': 'Requestion a new station',
      'StationRequestBody': 'StationRequestBody2'
    }
  },
  ro: {
    translation: {
      'How to Vote': 'Cum votezi',
      'Map': 'Harta',
      'About': 'Despre',
      'Vot Diaspora': 'Vot Diaspora',
      'MapMessage': 'Folosește harta de mai jos pentru a găsi secția de votare cea mai apropiată de tine.',
      'HeroBody': '<p>Pe 10 noiembrie 2019 au loc alegeri prezidențiale în România. Românii de pretutindeni au dreptul de a-și alege viitorul președinte. Dacă locuiești peste hotare poți vota prin corespondență sau la secția de votare. Descoperă cu ajutorul Vot Diaspora care sunt procedurile pentru a te înregistra să votezi prin corespondență sau de ce acte ai nevoie pentru a vota la secție.</p><p>Dacă ai decis să votezi la o secție, indiferent că ești rezident sau domiciliat într-o țară străină sau ești doar în tranzit, folosește harta de mai jos pentru a vedea care este cea mai apropiată secție de votare din orașul sau țara în care doreşti să votezi. Votul în diaspora pentru alegerile prezidențiale din 2019 se desfășoară pe parcursul a trei zile pentru fiecare tur de scrutin.</p>',
      'HowToVoteBody': `
        <p>
          Conform legislației electorale, dacă te afli în afara României ai două moduri prin care îți poți exprima votul: prin corespondență sau la secția de votare. Descoperă mai jos cele două opțiuni și pașii pe care trebuie să îi urmezi în fiecare dintre cazuri—
        </p>
        <h3>Prin corespondență</h3>
        <p>
          În ce condiții poți vota prin corespondență:
          <ol>
            <li>Ești cetățean român cu drept de vot</li>
            <li>Locuiești în străinătate și deții documente valabile de ședere din partea statului în care te afli</li>
          </ol>
        <p>
          Dacă îndeplinești aceste două condiții atunci pasul următor este să te înregistrezi ca alegător prin corespondență pe <a href="https://www.votstrainatate.ro/">website-ul pus la dispoziție de AEP</a> (Autoritatea Electorală Permanentă). Va trebui să le furnizezi printr-un formular o serie de informații cu caracter personal, precum și copii scanate sau fotografii ale actului de identitate şi documentului care face dovada dreptului de ședere.
        </p>
      <p>
        Înscrierea este valabilă pentru ambele tururi de scrutin (nu există opţiunea de a te înscrie să votezi prin corespondenţă numai la unul dintre tururi).
      </p>
      <p>
        Vei primi prin poștă toate documentele necesare (plicuri speciale, autocolantele cu care îți vei exprima votul, certificatele de alegător etc) pentru ambele tururi de scrutin și instrucțiunile de utilizare. Toate aceste documente va trebui să le depui în cutia poștală sau la un oficiu poștal conform cu instrucțiunile primite.
      </p>
      <p>
        Dacă te răzgândeşti şi alegi să votezi în persoană la secţia de votare după ce te-ai înscris ca alegător prin corespondenţă, va trebui să dai o declaraţie pe propria răspundere că nu ai votat prin corespondenţă. Votul prin corespondenţă se consideră exprimat în momentul în care biroul electoral primeşte plicul trimis de tine prin poştă; potrivit legii, după ce votul tău a fost primit prin poştă nu vei mai putea vota la secţia de votare, deoarece ai dreptul la un singur vot.
        </p>
      <p><strong>Termenul limită de înscriere ca alegător prin corespondență este 11 septembrie.</strong>
      </p>
      <h3>Vot la secție</h3>
      <p>
        Dacă vrei să votezi la o secție de votare, poți să te înscrii în registrul electoral pe <a href="http://www.votstrainatate.ro/">website-ul pus la dispoziție de AEP</a> (Autoritatea Electorală Permanentă). Va trebui să le furnizezi o serie de informații cu caracter personal: nume, prenume, CNP, precum și localitatea și statul unde vrei să votezi. În plus, va trebui să atașezi și o fotografie sau copie scanată a actului de identitate.
      </p>
      <p>
        Cetățenii români care se înscriu în registrul electoral vor fi înscriși pe liste permanente dedicate diasporei. Cei care nu se înscriu vor putea vota pe liste suplimentare. Dacă te-ai înscris în registrul electoral și ai declarat că vrei să votezi într-un anumit oraș, dar din orice motiv nu mai poți vota acolo, poți vota în orice secție pe listele suplimentare. <strong>Data limită de înscriere în registrul electoral este 11 septembrie.</strong>
      </p>
      <p>
        Votarea în străinătate se va desfășura pe parcursul a trei zile. Programul este următorul:
      </p>
      <ol>
        <li>Vineri între ora 12:00 (ora locală) și 21:00 (ora locală)</li>
        <li>Sâmbătă între ora 7:00 (ora locală) și 21:00 (ora locală)</li>
        <li>Duminică între ora 7:00 (ora locală) și 21:00 (ora locală)</li>
      </ol>
      <h4>Documente necesare</h4>
      <p>
        În ziua scrutinului trebuie să ai la tine, pentru a putea vota, unul din următoarele acte de identitate: cartea de identitate, cartea electronică de identitate, cartea de identitate provizorie, buletinul de identitate ori paşaportul diplomatic, paşaportul diplomatic electronic, paşaportul de serviciu, paşaportul de serviciu electronic, paşaportul simplu, paşaportul simplu electronic, paşaportul simplu temporar, titlul de călătorie, iar, în cazul elevilor din şcolile militare, carnetul de serviciu militar, valabile în ziua votării.
      </p>`,
      'Map of Polling Stations Abroad': 'Harta secțiilor de votare',
      'Requestion a new station': 'Înființarea unei noi secții',
      'StationRequestBody': '<p>Înscrierea ta în registrul electoral ca cetățean care vrea să voteze în străinătate are și valoare de cerere de înființare de secție de votare. Autoritatea Electorala Permanentă poate cere Ministerului Afacerilor Externe înființarea de secții de votare suplimentare pe lângă cele constituite, de regulă, în sediile misiunilor diplomatice și ale oficiilor consulare ale României din diaspora. Cu acordul autorităților din țara respectivă, AEP poate cere acest lucru pentru localitățile sau grupurile de localități unde au înregistrat cel puțin 100 de alegători. După consultări, AEP stabilește sediile secțiilor de votare din străinătate. Pentru detalii suplimentare, puteți consulta Legea 148/2019, Articolele 1-8 aici.</p>'
    }
  }
};
